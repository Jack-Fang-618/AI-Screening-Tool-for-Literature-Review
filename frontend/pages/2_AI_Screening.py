...
def show_progress_tracker():
    ...
    try:
        ...
        # Dual-AI Strategy Effect (Consensus vs Arbiter)
        consensus_count = status.get('consensus_count', 0)
        arbiter_count = status.get('arbiter_count', 0)
        
        if consensus_count > 0 or arbiter_count > 0:
            st.markdown("### 🤖 Dual-AI Effect")
            
            c1, c2 = st.columns(2)
            with c1:
                st.metric("Consensus ✅", f"{consensus_count}")
                st.caption("Both fast agents agreed")
            with c2:
                st.metric("Arbiter ⚖️", f"{arbiter_count}")
                st.caption("Expert intervention needed")
            
            if (consensus_count + arbiter_count) > 0:
                rate = (consensus_count / (consensus_count + arbiter_count)) * 100
                st.progress(rate / 100)
                st.caption(f"Consensus Rate: {rate:.1f}%")
...
- Pediatric populations""",
    ...
    strategy = st.radio(
        "Screening Strategy",
        options=["single", "dual_arbiter"],
        index=0,
        format_func=lambda x: "Standard (Single Model)" if x == "single" else "High Quality (Dual-Agent + Arbiter)",
        help="Standard is faster and cheaper. High Quality uses two models and a third 'Arbiter' to resolve conflicts."
    )
    
    if strategy == "dual_arbiter":
        st.info("⚖️ **Dual-Agent + Arbiter active**: Two parallel agents will screen each article. If they disagree, a stronger expert model (Grok-4) will resolve the conflict. Note: This will increase cost but significantly improve reliability.")
    ...
    with col1:
        model = st.selectbox(
            ...
            index=0,
            help="Grok 4.1 Fast series models are the latest versions."
        )
    ...
    if st.button("📊 Calculate Accurate Cost Estimate", type="secondary", width="stretch"):
        with st.spinner("Analyzing dataset and calculating cost..."):
            try:
                ...
                # dual_arbiter: runs 2 passes, plus one stronger model (~1000 tokens) if disagree
                # We assume 10% disagreement for conservative estimation
                passes = 2 if strategy == "dual_arbiter" else 1
                disagreement_rate = 0.10 if strategy == "dual_arbiter" else 0.0
                ...
                # First article cost is higher because passes/arbiters happen
                if strategy == "dual_arbiter":
                    # Two passes (first pass already calculated) + arbiter overhead
                    first_article_cost = (first_article_cost * 2) + (1500 * input_price_per_1m / 1_000_000)
                ...
                # Apply strategy multiplier to subsequent articles
                if strategy == "dual_arbiter":
                    # 2 passes + roughly 10% of articles needing arbiter
                    subsequent_article_cost = (subsequent_article_cost * 2) + \
                                            (disagreement_rate * 2500 * input_price_per_1m / 1_000_000)
